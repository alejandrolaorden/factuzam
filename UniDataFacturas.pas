...
begin
  inherited;
  if (unqryLinFac.FindField('LINEA_FACTURA_LINEA').AsString  = '0') then
  begin
    ...
    unqryLinFac.FindField('LINEA_FACTURA_LINEA').AsString :=
...
procedure TdmFacturas.unqryTablaGAfterDelete(DataSet: TDataSet);
  var
...
begin
  ...
  with unqryTablaG do
  begin
    if (FieldByName('RAZONSOCIAL_CLIENTE_FACTURA').AsString = '') then
    begin
      raise EDatabaseError.CreateFmt('Debe escribir la razón social del' +
                                                     ' cliente a facturar ',[]);
      Abort;
    end;
    if (FieldByName('RAZONSOCIAL_EMPRESA_FACTURA').AsSTring = '') then
    begin
      raise EDatabaseError.CreateFmt('Debe escribir la razón social de ' +
                                                  ' la empresa a facturar ',[]);
      Abort;
    end;
    if (FieldByName('SERIE_FACTURA').AsString = '') then
    begin
      raise EDatabaseError.CreateFmt('Debe seleccionar una serie ' +
                                                           'para facturar ',[]);
      Abort;
    end;
