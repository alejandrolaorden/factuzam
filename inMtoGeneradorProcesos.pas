...
type
  ...
  procedure ShowMtoGeneradorProcesos(Owner       : TComponent); overload;
  procedure ShowMtoGeneradorProcesos(Owner       : TComponent; sOdon: String); overload;
const
...
begin
   frmMtoGeneradorProcesos :=
               TfrmMtoGeneradorProcesos(FindMDIChildOpen((Owner as TfrmOpenApp),
                                                       TfrmMtoGeneradorProcesos,
                                                    'frmMtoGeneradorProcesos'));
  if (frmMtoGeneradorProcesos = nil) then
  begin
    frmMtoGeneradorProcesos := TfrmMtoGeneradorProcesos.Create(Owner);
    frmMtoGeneradorProcesos.edtBusqGlobal.SetFocus;
  end
  else
    frmMtoGeneradorProcesos.BringToFront;
end;
...
begin
  ...
  begin
    ...
    frmMtoGeneradorProcesos.BringToFront;
   if not (frmMtoGeneradorProcesos.tdmDataModule.unqryTablaG.Locate(pkFieldName,
                                                           sOdon, [])) then
    ShowMessage('Proceso no encontrado')
   else
     frmMtoGeneradorProcesos.pcPantalla.ActivePage :=
                                                frmMtoGeneradorProcesos.tsFicha;
end;
